// v1.5
// added support to turn off obstacle detection.
...
{
  if (upButton && !downButton) {runMotors();}
...
void loop()
{
  ...
  speedControl();
  printStatus();  
  if (IROff) {motorsControlWithoutIR();}
  else {motorsControlWithIR();}
  Serial.println(String("IR: ") + !IROff);
}
